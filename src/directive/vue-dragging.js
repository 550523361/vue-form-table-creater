...
export default function (Vue, options) {
    ...
    function handleDragEnter(e) {
        ...
        const key = el.getAttribute('drag_group')
        if (!Current.el || !Current.item || el === Current.el) return;//key !== Current.group || 可以跨组拖拽
        const drag_key = el.getAttribute('drag_key')
        const DDD = dragData.new(key)
        const item = DDD.KEY_MAP[drag_key]
    ...
    function handleDragEnd (e) {
        ...
        const groupList = el.getAttribute('drag_list')
    ...
    function addDragItem (el, binding, vnode) {
        ...
        if(el.getAttribute("disabledrag")){
           // 不允许拖拽
        }else{
       ...
       // el.setAttribute('drag_list', JSON.stringify(item))
